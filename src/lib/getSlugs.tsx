...
export const getPostSlugs = (posts: contentful.post[]): getSlugs.postSlugs => {
  const postSlugs = Array.from({ length: posts.length }, (_, i) => {
    return {
...
// page slug一覧
export const getPageSlugs = (pages: contentful.page[]): getSlugs.pageSlugs => {
  let pageSlugs = Array.from({ length: pages.length }, (_, i) => {
    return {
...
// postList slug一覧
export const getPostListSlugs = (postListNumPages: number): string[] => {
...
export const getCategorySlugs = (
  ...
  const categorySlugs = Array.from({ length: allCategory.length }, (_, i) => {
    return {
...
// Tag slug一覧
export const getTagSlugs = (tags: contentful.tags[]): getSlugs.catTagslugs => {
  const tagSlugs = Array.from({ length: tags.length }, (_, i) => {
    return {
